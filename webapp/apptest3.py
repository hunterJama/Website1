...
def is_strong_bearish(candle):
    ...
    high_price = float(candle['mid']['h'])
    low_price = float(candle['mid']['l'])
    return (open_price - close_price) > (high_price - low_price) * 0.2
...
def run_trade_bot():
    ...
    while running ==True and entry == 0:
        if entry==0:
            enter_buy()
            buy_entry1=1
        else:
...
def start_tradebot():
    ...
    return "TradeBot started successfully!"

@app.route("/stop-trade-bot")
def stop_trade_bot():
    global running
    running = False

    return "TradeBot stopped successfully"
